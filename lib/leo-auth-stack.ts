...
import { Table, AttributeType, StreamViewType, ProjectionType } from '@aws-cdk/aws-dynamodb'
import { Role, ServicePrincipal, AccountRootPrincipal, ManagedPolicy, PolicyDocument, PolicyStatement } from '@aws-cdk/aws-iam'
import { CustomResource, Stack, Construct, StackProps, CfnOutput, Fn, CfnParameter, Duration, RemovalPolicy } from '@aws-cdk/core'
...
export class LeoAuthStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    apiRoleAssumePolicy.addToPrincipalPolicy(new PolicyStatement({
    ...
    const resources = "{\"region\":\""+this.region+"\",\"LeoAuth\":\""+leoAuth.tableName+"\",\"LeoAuthUser\":\""+leoAuthUser.tableName+"\",\"LeoAuthPolicy\":\""+leoAuthPolicy.tableName+"\",\"LeoAuthIdentity\":\""+leoAuthIdentity.tableName+"\"}"
    const environment = {
      Resources: resources,
    ...
    const normalizeDataLambda = new NodejsFunction(this, 'NormalizeData', {
      ...
      environment,
      role: leoAuthRole,
      bundling: { nodeModules: ['leo-config', 'leo-auth'], externalModules: ['aws-sdk',] }
    });
    ...
    const leoAuthManagedPolicy = new ManagedPolicy(this, "LeoAuthManagedPolicy", { //TODO: is this a managed policy?
      statements: [
        new PolicyStatement({
    ...
    new CustomResource(this, 'CustomSeedDatabase', { serviceToken: myProvider.serviceToken });
          ...
          "/api/authorize": {
            "put": {
              ...
              "x-amazon-apigateway-integration": {
                ...
                "uri": 'arn:aws:apigateway:' + this.region + ':lambda:path/2015-03-31/functions/' + authorize.functionArn + '/invocations',
